...
object MusicPlayer {
    ...
    enum class RepeatMode { OFF, ONE, THREE, ALL }
    val repeatMode = mutableStateOf(RepeatMode.ALL)
    private var repeatCounter = mutableStateOf(0)
    ...
    fun cycleRepeatMode() {
        repeatMode.value = when (repeatMode.value) {
            ...
            RepeatMode.ONE -> RepeatMode.THREE
            RepeatMode.THREE -> RepeatMode.ALL
    ...
    private fun updateExoPlayerRepeatMode() {
        exoPlayer.repeatMode = when (repeatMode.value) {
            ...
            RepeatMode.THREE -> Player.REPEAT_MODE_OFF
    ...
    private fun handleTrackEnded() {
        when (repeatMode.value) {
            RepeatMode.OFF -> {
                if (currentTrackIndex.value < playlist.size - 1) {
                    next()
                }
            }
            RepeatMode.ONE -> {
    ...
    fun getRepeatCounterDisplay(): String {
        return when (repeatMode.value) {
            RepeatMode.THREE -> "${repeatCounter.value + 1}/3"
            else -> ""
        }
    }
