...
class VoiceAssistant(private val context: Context) : RecognitionListener {
    ...
    private fun initModel() {
        scope.launch(Dispatchers.IO) {
            try {
                // Пытаемся принудительно загрузить библиотеку, если JNA в JAR не подхватывается автоматически
                try {
                    System.loadLibrary("jnidispatch")
                } catch (e: UnsatisfiedLinkError) {
                    Log.w("VoiceAssistant", "System.loadLibrary(jnidispatch) failed, relying on JNA default loading")
                }
                
                System.setProperty("jna.nosys", "false")
                
                val modelDirName = "vosk-model-small-ru"
                val targetDir = File(context.filesDir, "model") // Use 'model' as in old project
                ...
                // Initialize recording like in old VoiceProcessor
